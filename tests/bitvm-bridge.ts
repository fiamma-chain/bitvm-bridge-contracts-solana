...
describe("Test Bitvm Bridge", async () => {
  ...
  const btcLightClientProgram = new anchor.Program<BtcLightClient>(btcLightClientIDL, provider);
  const bitvmBridgeProgram = new anchor.Program<BitvmBridge>(bitvmBridgeIDL, provider);
  ...
  it("Initialize BTC Light Client", async () => {
    ...
    const genesisBlock = {
      height: 230627,
      hash: Buffer.from("35c40037f72c8a014c431212ad9d7452682243e5fa5de4bc4548550ac2000000", "hex"),
      time: 1736757020,
      target: Buffer.from("0003400100000000000000000000000000000000000000000000000000000000", "hex"),
    };
    ...
    const state = await btcLightClientProgram.account.btcLightClientState.fetch(statePda);
    ...
    expect(Buffer.from(state.latestBlockHash).toString('hex')).to.equal(Buffer.from(genesisBlock.hash).toString('hex'));
    expect(Buffer.from(state.latestPeriodTarget).toString('hex')).to.equal(Buffer.from(genesisBlock.target).toString('hex'));
  ...
  it("Verify tx", async () => {
    ...
    const blockHeader = Buffer.from("0000002005cd6ba37338a8c37e431180aa2d2175c39d9a6bdf3954653cb0630420000000a553f5c27318e0d7afbcf242942a0fd099683a6e6670f4a68dc8ab9ddfd7761a1ccf8467ad46011edb6c4401", "hex");
    const txId = Buffer.from("c6c911614166de26173be7c90ba37a0a26c44c3dac9bb69f84ef5b35d7525026", "hex");

    const rawTx = Buffer.from("", "hex");

    const txIndex = 378;
    const outputIndex = 0;
    const expectedAmount = 100000;
    const expectedScriptHash = Buffer.from("85f1940c71a1e1a852db646fa0f79cf1e5defc9e4bda671ad4cf9000ada74b41", "hex");
    const txMerkleProof = [
      Buffer.from("d1263d3e754e1167d9a68b7c4ca98b245b696ecc18badea92e49c55c0729bd1a", "hex"),
      Buffer.from("20aeeae156d22ffb9a128ede8555de27c3c6bd9f47647e3de7bd4b332f8d5086", "hex"),
      Buffer.from("67e6ea2dd621b45e7aa1ff1d20977851b4694c25db628bc1359072e83fe0a2af", "hex"),
      Buffer.from("83579ca714156e1ad2832cbc4b0c1ca2d599e2cc46f6d31e967c844415c6767a", "hex"),
      Buffer.from("17fc9275ca3b6d678b77ab3677a7da37c65997893f7c10db67b5552c628f7d7f", "hex"),
      Buffer.from("50c7a085616cca1e17a766a1c581eede0139f5b0c03a8e37dfe3c2d8c2798e0a", "hex"),
      Buffer.from("239c7c1ea91e3868721ef686ca893a40b1d532d637a3fefd53df1de9aba847ab", "hex"),
      Buffer.from("0a117857964bc8182f3ddc1cdda9c71b3a081989d0aa3404be924ea3c1507671", "hex"),
      Buffer.from("23fae37f988d3dcc5cf4ed21139d9e9c3b35d68b2c1cbfaf21b67fb606cd0954", "hex"),
      Buffer.from("7a4491c685b8ae32f9ae1266a193ac335ca8f722c47af58d4e2c8283892ed091", "hex"),
    ]
    await btcLightClientProgram.methods
      .verifyTransaction(
      ...
      .rpc();
    const txState = await btcLightClientProgram.account.txVerifiedState.fetch(txVerifiedStatePda);
  ...
  it("Initialize an SPL Token", async () => {
    ...
    const state = await bitvmBridgeProgram.account.bridgeState.fetch(bridgeStatePda);
  ...
  it("Mint some tokens to your wallet!", async () => {
    ...
    const newbtcTxState = await bitvmBridgeProgram.account.txMintedState.fetch(txMintedStatePda);
  ...
  it("Transfer some tokens to another wallet!", async () => {
    ...
    const senderAccount = await provider.connection.getAccountInfo(senderTokenAccountAddress);
    const recipientAccount = await provider.connection.getAccountInfo(recipientTokenAccountAddress);

    // parse token account data
    const senderTokenAccount = await unpackAccount(senderTokenAccountAddress, senderAccount);
    const recipientTokenAccount = await unpackAccount(recipientTokenAccountAddress, recipientAccount);
  ...
  it("Non-owner performs mint should fail!", async () => {
    ...
    try {
      await bitvmBridgeProgram.methods.mint(Array.from(txId), amount)
        .accounts({
